...
- The shared repository is `https://github.com/Fenglin-Maple/Blibili-Markdowns`. Contributors are represented by GitHub numeric ID and stable document/source IDs, not by a mutable display name. The application opens the default browser for GitHub access, reads the credential through the project-local Git Credential Manager, then uses the project-local `runtime/git` to create a Fork branch and Pull Request. A pasted Fine-grained Token remains a fallback; no global Git credential or configuration is accepted.
