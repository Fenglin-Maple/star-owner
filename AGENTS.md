...
- The shared repository is `https://github.com/Fenglin-Maple/Blibili-Markdowns`. Contributors are represented by GitHub numeric ID and stable document/source IDs, not by a mutable display name. The application opens the default browser for GitHub access and stores the credential in a project-private DPAPI store used only by bundled `runtime/git`; a pasted Fine-grained Token remains a fallback. Ordinary users create a Fork branch and Pull Request, while the repository owner creates a temporary upstream branch and PR. Commit authors use the authenticated GitHub noreply identity. Clearing authorization removes only application-owned credentials and must never erase Windows Credential Manager, system Git or user-global Git data.
