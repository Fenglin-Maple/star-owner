...
- **1.6.3**：新增未来功能候选备忘录，记录 RAG 笔记、按需查看互联网图片、文件/代码/Skill 操作、MCP 管理和可安装工具生态的后续设计边界；不改变当前运行时、依赖或已实现功能。

- **1.6.2**：修复 RAG 助手读取网页时的取消链路。停止生成会中止 DNS、网页加载、动态正文等待、隐藏窗口和临时代理；网页图片直链会快速提示当前仅支持文本/链接，不再反复请求查看像素；取消后的同一会话可以继续交流。新增取消恢复、图片直链和图片内容类型回归测试。
...
- **1.2.x - 1.3.x**：上线“B站之外”工具箱，提供单文件音视频字幕、本地媒体压缩导入和多模态文档知识库导入；媒体工具统一进入资源队列，并加强大文件限制、收藏夹事务、取消回滚、更新恢复、网络边界和运行时隔离。
- **1.4.x**：加入多 P 视频父任务、逐 P 进度与续做，以及基于 Portable Git、Pull Request 和 `catalog.json` 的 B站总结共享；共享文档挂载到独立“共享”用户并进入文档库与 RAG。同期完成 AI 空响应/限流重试、ASR 时间轴归一化与三次识别兜底、共享挂载事务回滚和长任务界面性能优化。
- **1.5.0 - 1.5.1**：整理共享工具、RAG 知识库选择器、本地工具和应用内 README 的布局与安全渲染；优化启动页空间、共享目录操作和标题栏随机字标，保持任务、IPC、数据库及共享仓库契约不变。
- **1.5.2**：修复本地导入媒体被时长筛选误隐藏、本地视频总结标题格式导致的产物误判，以及 RAG 原图重复加载、超大 Base64 被误算为文本上下文并导致多模态查看失败的问题。
...
- **1.5.7**：RAG 将最终回答前的模型过渡文字和工具调用按实际顺序归入较淡的“过程内容”折叠区；最新回答生成时自动展开，完成后和历史轮次默认收起。旧会话缺少调用位置时恢复为工具记录在正文开头，且不拆分既有正文。
- **1.5.8**：统一 OpenAI 工具调用与多模态图片消息顺序，兼容 MiniMax 等严格接口；共享仓库将 `validate-shared-docs` 设为合并前必需检查，并保留仓主已有分支规则；多 P 按真实 `index.md/summary.md` 产物重算完成状态、隔离父任务上传范围并防止旧清理尝试误删新产物；修复本地导入视频小数时长导致文档库列表被筛空。
...
- “GitHub 文档共享”只允许已完成的 B站视频总结（多 P 必须上传完整父任务包），不上传原始视频、音频、ASR 缓存、Cookie 或 API Key。上传者可按用户、收藏夹、标题/BV/UP主、生成时间和时长筛选，批量加入按收藏夹折叠的准备上传列表，并可在上传栏显式刷新本地目录。默认浏览器授权存入项目私有 DPAPI 凭据目录，Token 粘贴保留为备用；普通用户创建 Fork/分支/PR，仓库主人直接创建仓库分支/PR。共享仓库会把稳定的 `validate-shared-docs` Action 检查设置为合并前必需检查；仓主首次连接或一键建仓需要具备该仓库的分支管理权限（Fine-grained Token 需 Administration: Read and write，经典 Token 需对应 repo 管理权限）。清除授权只删除星藏家数据库和内置 Git 私有凭据，不修改系统 Git 或全局凭据。连接仓库前会验证共享规范并加入已验证仓库列表；远程目录优先一次读取 Action 生成的 `catalog.json`，挂载更新通过内置 Git 的 partial clone + sparse checkout 批量导入，并在整批失败时恢复原状态。下载者可按 GitHub 用户、B站用户和视频名称搜索四层远程目录，将筛选结果、单篇或整个远程收藏夹挂载到“共享”用户，也可多选本地挂载统一同步；挂载目标菜单可以选择已有收藏夹或直接输入新名称，未选择时会按日期时间自动创建本地共享收藏夹。同一远程收藏夹从首篇文档起就复用唯一来源节点，单篇、多篇和完整收藏夹操作只改变同步范围。无变化时不会重复下载。远程删除只标记失效并保留本地文档，共享收藏夹不进入任务总览或 Agent 派发。点击入口卡后进入独占工具视图，常规宽度下左侧下载/挂载、右侧上传 PR；各项远程自动化均显示进度。
