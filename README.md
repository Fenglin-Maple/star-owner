...
5. 应用第一次强制不携带 Cookie，优先尝试公开获取视频与元数据；只有工具明确返回登录、注册用户、私有视频或 Cookie 必需信号时，才进入“等待登录”并弹窗提供 B站登录跳转；
6. 登录完成后回到单视频页面点击“开始/继续”，应用同步新的 Cookie 并重试；
7. 界面流式显示模型推理字段、正文、阶段、进度、日志和 Token；
8. 通过校验后，完整视频产物先归档到默认 Workspace 的 `内置用户/<内置收藏夹>/`，再复制一份到指定外部目录。
