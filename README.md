...
持续接单会话不会把上一条视频的模型消息带入下一条：每次领取视频都会在相同 Worker ID 下创建新的独立模型上下文，只有累计 Token 统计和工作记录跨任务保留，每个任务仍获得新的 `workId`。应用按模型配置的上下文窗口预留协议、图片和输出空间；超长字幕、素材或修订稿会按结构化章节自动压缩，并显示当前独立上下文轮次、预计占用和压缩次数。供应商仍返回上下文超限时，应用保留 Worker ID、`workId` 和当前任务，以更紧凑且不携带旧请求历史的全新请求自动重试一次；若仍失败，则按普通任务失败回滚并清理缓存。
...
4. 可选择“处理完成后保留缓存视频到视频文件夹”，默认关闭；未启用任何远程模型时启动按钮不可点击；
5. 应用第一次强制不携带 Cookie，优先尝试公开获取视频与元数据；只有工具明确返回登录、注册用户、私有视频或 Cookie 必需信号时，才进入“等待登录”并弹窗提供 B站登录跳转；
6. 登录完成后回到单视频页面点击“登录后重试”，应用同步新的 Cookie 并从头处理；
7. 界面流式显示模型推理字段、正文、阶段、进度、日志、累计 Token 和当前独立上下文预算；
8. 通过校验后，完整视频产物直接归档到默认 Workspace 的 `内置用户/<内置收藏夹>/<视频产物目录>/`，不再要求或生成外部目录副本；表单可打开收藏夹输出目录，完成详情可打开本次产物目录。
