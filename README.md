...
`1.0.2` 是当前稳定维护版。应用核心使用 `1.0.2`，项目内运行时与 faster-whisper `small` / `medium` 权重固定复用 `v1.0.0` Release 依赖基线；只有依赖内容或目录契约实际变化时才发布新的依赖包。本版增强 GitHub 依赖下载的自动重试、断点续传、完整包复用和 SHA-256 digest 校验，并在安装后立即刷新 ASR 与工具状态。
...
- 使用独立、持久化、沙箱化 WebView 登录 Bilibili，不复用系统浏览器登录态；支持扫码登录，WebView 中点击视频会在共享同一登录 Session 的独立窗口中打开。
