...
已有 v1.0.3 或更高版本时，也可以从最新 Release 只下载 `Star-Owner-Updater-v<version>-win-x64.exe`。先关闭旧星藏家和全部 Agent，双击更新器并选择包含 `Start-StarOwner.cmd` 的旧项目根目录；更新器会自动安装 GitHub `latest` 正式版到原目录。不要先删除或移动 `workspace/`，原路径更新才能完整继承收藏夹、RAG 文档、模型、缓存、B站登录和应用私有 GitHub 凭据。
