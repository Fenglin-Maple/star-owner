...
- **1.0.3 稳定基线**：完善模型配置刷新与任务治理，固定由应用内 Agent 执行视频处理；RAG 和外部 Agent 以已完成知识库为边界，减少旧前端状态造成的误判。
- **1.0.4 - 1.1.x**：补强 Windows 路径、日志和项目内运行时隔离；引入低显存 `large-v3-turbo`、独立 CPU ASR、主题与设置优化，并加入更新迁移、收藏夹变化提醒和任务筛选启用。
- **1.2.x - 1.3.x**：上线“B站之外”工具箱，支持本地音视频字幕、媒体压缩导入、多模态文档知识库导入和多 P 视频总结；统一资源排队、取消回滚、磁盘空间、大文件和运行时安全边界。
- **1.4.x**：完善多 P 父任务、逐 P 进度与续做；建立基于 Portable Git、Fork/分支、Pull Request、校验 Action 和 `catalog.json` 的 B站总结共享，挂载内容进入独立“共享”用户并支持 RAG。同期补强 AI 空响应/限流重试、ASR 时间轴归一化、共享事务和长任务性能。
- **1.5.x**：持续优化共享工具、RAG 检索与多模态图片调用，支持更大的检索预算、按收藏夹定位、过程消息折叠和图片批处理；修复严格工具调用兼容、图片索引、网页访问、本地导入、共享目录 Action 与多 P 完成状态等问题，并完善共享设计文档。
- **1.6.0 - 1.6.5**：完成运行时依赖、更新迁移和安装事务的隔离审计，强化 portable manifest、回滚和 Windows 系统工具边界；修复 RAG 网页取消、隐藏浏览器连接和动态正文等待问题；多 P 工作流补齐登录 Cookie 传递、父元数据复用、B站 `412/-412` 风控退避及高频进度性能治理，并整理后续 RAG、MCP、文件操作及工具生态的规划边界。
