...
- **1.7.5 · B站收藏夹元数据加固**：Agent 处理真实 B站收藏夹来源任务时，读取视频信息若 `pages` 缺失、为空或与 `videos` 数量不一致，最多进行三次总请求；仍不完整就清理本次尝试、关闭并跳过该任务，同时保留手动重新启用后的重试入口。单视频、多 P 专用工具、本地导入和下载缓存不套用这项 Agent 专用校验。
...
- 对真实 B站收藏夹来源的 Agent 任务，视频元数据阶段会检查 `pages` 是否完整；缺失、为空或数量与 `videos` 不一致时最多尝试三次，仍未恢复则将任务关闭并跳过，后续任务继续处理。该校验只绑定 B站收藏夹来源；单视频内置收藏夹、多 P 专用收藏夹、本地视频/音频导入和 B站下载缓存保持各自原有路径。
