...
> `1.5.1` 版本边界：视频总结任务只由应用内 Agent 工作流执行。外部 Codex、Claude Code、OpenCode 或其它 Agent 不再领取视频任务，也不能调用媒体工具或提交产物；它们可以通过本机只读 HTTP API 访问全部已完成 Markdown 知识库。
>
...
启动页的“最近状态”会在宽窗口中填补剩余高度，但不会压缩“第一次上手”“工具接口状态”等固定内容；展开上方说明后仍保留可读的默认高度并允许页面滚动。标题栏每次启动会从六套“星藏家 Star-Owner”“星 Star-Owner”“星 Owner”字标中随机选择一套，中文使用中等字重黑体并按标题栏明暗切换黑白，英文使用对应主题强调色；其中一套保留手绘 SVG 下划线。

GitHub 文档共享工具进一步统一两栏标题、说明、阴影、主题色顶部边框、四组列表背景和纵向位置。筛选输入保持紧凑并始终显示下拉箭头，主要操作采用固定宽度的两行布局；上传栏可以显式刷新本地目录。挂载目标改为下拉菜单内创建新共享收藏夹，未选择目标时会在挂载时使用日期时间名称自动创建。三个本地导入工具保持居中标题、左上角返回按钮和受限内容宽度。上述变化只涉及 Renderer 布局与现有共享接口的前端调用，不改变任务、共享仓库、挂载来源、上传事务、IPC 或数据库契约。
