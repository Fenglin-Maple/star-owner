...
- **1.6.0 - 1.6.8**：完成运行时依赖、更新迁移和安装事务的隔离审计，强化 portable manifest、回滚和 Windows 系统工具边界；修复 RAG 网页取消、隐藏浏览器连接和动态正文等待问题；多 P 工作流补齐登录 Cookie 传递、父元数据复用、B站 `412/-412` 风控退避、高频进度性能治理、父任务删除与高 P 启动批量落盘、局部父任务事件和列表滚动保持；单视频、多 P 与 B站视频缓存不再执行匿名首次请求，联网前会校验当前登录 Cookie，缺失或失效时给出明确提示；并整理后续 RAG、MCP、文件操作及工具生态的规划边界。
