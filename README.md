...
- “批量下载视频缓存”可多次提交 BV 号、完整链接或短链，按队列并行下载合轨视频；公开访问失败且明确需要登录时才暂停等待。
- 内置缓存视频收藏夹可直接在“任务激活”中成为 Agent 工作源，素材工具自动复用已有 `merged.*`，不会重复下载。
- “视频库”提供收藏夹、标题、BV、UP、标签、时长和下载日期筛选，以及主题化播放器、文件定位、缺失检测和确认删除。
...
2. 在“收藏夹同步”中读取列表并同步目标收藏夹，应用建立或增量更新任务库存；也可以在“批量下载视频缓存”中先建立本地缓存库存。
...
缓存视频同样登记在 SQLite，并默认保存在 `<workspace>/内置用户/<缓存收藏夹>/视频缓存/`。用户也可以为一次下载批次选择自定义输出根目录；应用会把该根目录和每个视频目录一起登记，用于后续 Agent 复用和删除边界校验。默认“内置视频缓存”收藏夹受保护且不能删除，其它缓存收藏夹删除前必须确认，并只会删除通过登记根目录校验的视频目录。
...
“AI 模型配置”是三类 AI 功能共用的模型中心。用户可以保存多个 OpenAI-compatible 或 NewAPI-compatible 供应商，拉取模型列表，启用多个模型，并声明上下文、工具、推理流和视觉等能力。API Key 由主进程加密保存；视频总结 Agent 和 RAG 知识库助手读取同一份配置与按模型 Token 统计。
...
“批量下载视频缓存”不依赖 AI 模型。每次提交必须选择一个内置缓存视频收藏夹，可现场新建；输入支持 BV 号、普通视频链接和 Bilibili 短链。下载管理器先读取公开元数据，再通过应用工具队列下载 720p 以内的合轨视频，保存标题、UP、时长、标签、发布时间、下载时间和文件位置。只有明确需要登录时才暂停并弹出登录入口，登录后可一次恢复全部等待项。

缓存收藏夹会出现在“任务激活”的内置用户下。激活后，内外部 Agent 领取到的是普通任务协议，但任务额外带有 `cachedVideoId`、`cachedVideoFile` 和 `reuseCachedMedia`。素材包直接复用缓存视频，ASR、字幕、关键帧和评论仍按完整总结标准执行。Agent 仍调用 `clean-cache`，应用会保留合轨视频，只删除音频等过渡文件。

“文件浏览 → 视频库”使用自定义主题播放器，不启用浏览器原生 controls。列表会实时标记缓存文件是否存在，支持按缓存收藏夹、标题/BV/UP/标签、时长和下载日期筛选。可在资源管理器中定位单个视频；多选删除和收藏夹删除均需要确认。默认“内置视频缓存”收藏夹不可删除，后端还会校验登记根目录，避免删除到缓存目录以外。
