...
- 附件：文本、Markdown、JSON、CSV、代码、PDF 和 DOCX 会提取文字；兼容模型可接收图片和音频。文件选择器导入或在输入框按 `Ctrl+V` 粘贴的剪贴板图片会先保存到会话沙盒，发送前显示缩略预览，发送后在用户消息中保留原图预览，并作为多模态输入交给启用视觉能力的模型。视频及其它不属于标准兼容接口输入的文件保留在会话沙盒中供文件工具处理。
