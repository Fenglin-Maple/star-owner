...
const { ASR_MODELS, asrComputeType } = require('../src/core/asr-models');
...
(async () => {
  ...
  for (const model of ASR_MODELS) {
    const modelPath = path.join(PROJECT_ROOT, 'runtime', 'models', model.id, 'model.bin');
    if (!fs.existsSync(modelPath)) {
      if (model.required) throw new Error(`Required ASR model is missing: ${model.id}`);
...
async function verifyModel(model, audio, root, requestCount) {
  const computeType = asrComputeType(model, 'cuda');
  const service = new AsrService({ id: `asr-gpu-${model}-test`, device: 'cuda', computeType, model });
  try {
    await service.start();
    ...
    assert.strictEqual(service.status().computeType, computeType, `${model} used an unexpected compute type`);
    assert(pid && results.every((item) => item.ok && item.computeType === computeType && item.diagnostics && Number.isInteger(item.diagnostics.sentenceCount)), `${model} ASR service request failed`);
    console.log(`persistent ASR ${model} ok, compute=${computeType}, pid=${pid}, requests=${requestCount}`);
  } finally {
    service.stop();
