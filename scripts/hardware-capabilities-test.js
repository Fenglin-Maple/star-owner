...
(async () => {
  ...
  const nvidia = evaluateAsrHardware({
    ...
    gpu: { available: true, name: 'RTX test', totalMiB: 8192 }, model: 'large-v3-turbo', pythonAvailable: true, runtimeHealth: runtime
  });
  ...
  const cpuOnly = evaluateAsrHardware({
    ...
    gpu: { available: false }, model: 'large-v3-turbo', pythonAvailable: true, runtimeHealth: { ...runtime, cudaDevices: 0 }
  });
  ...
  const missingCuda = evaluateAsrHardware({
    ...
    gpu: { available: true, name: 'RTX without CUDA', totalMiB: 8192 }, model: 'large-v3-turbo', pythonAvailable: true, runtimeHealth: { ...runtime, cudaDevices: 0 }
  });
  ...
  const undersizedModel = evaluateAsrHardware({
  ...
  assert(smallModel.nvidia.supported && !undersizedModel.localAsrSupported, 'model-specific GPU/RAM requirements were not enforced');
  ...
  const missingRuntime = await detectAsrHardware({ projectRoot: emptyProject, gpu: { available: false }, model: 'large-v3-turbo' });
