...
(async () => {
  ...
  assert(main.includes("if (event.type !== 'multipart-progress') publishEvent(event);") && main.includes("'multipart-progress', 'shared-upload-progress'") && main.includes('const { cacheState, localToolbox, multiPart, ...activity }'), 'relayed multi-part progress can still enter global activity persistence');
  assert(app.includes("'multipart-progress'") && outside.includes("if (event.type === 'multipart-progress') updateMultipartProgressView();"), 'multi-part progress still invalidates the global snapshot or rebuilds the complete tool UI');
  ...
  assert(multipartManager.includes("this.state({ reconcile: false })") && multipartManager.includes('partTasksByParent') && multipartManager.includes('activeSessionsByParent'), 'live multi-part state still performs full artifact reconciliation or repeated store scans');
