...
(async () => {
  ...
  assert(outsideCss.includes('.shared-upload-list { height: auto;') && outsideCss.includes('grid-template-rows: subgrid') && outsideCss.includes('.shared-layout > .outside-pane > .shared-flow-arrow') && outsideCss.includes('.shared-repository-summary') && outsideCss.includes('.shared-repository-card[open]') && outsideCss.includes('.shared-pane-title-copy > small') && outsideCss.includes('border-top-width: 2px') && outsideCss.includes('.shared-pane-target { padding-top: 10px; border-top: 1px solid var(--line); }') && /display:\s*flex;\s*flex-direction:\s*column;\s*grid-row:\s*auto;/.test(outsideCss) && outsideCss.includes('--shared-source-list-height') && outsideCss.includes('--shared-target-list-height') && outside.includes('sharedUploadTree(visibleItems') && outside.includes('sharedUploadTree(items'), 'shared directories lost the aligned flow tracks, responsive stacking, aligned target headings, or collapsible repository configuration');
