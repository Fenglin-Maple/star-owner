...
(async () => {
  ...
  assert(outsideCss.includes('.shared-upload-list { height: auto;') && outsideCss.includes('grid-template-rows: subgrid') && outsideCss.includes('.shared-layout > .outside-pane > .shared-flow-arrow') && /display:\s*flex;\s*flex-direction:\s*column;\s*grid-row:\s*auto;/.test(outsideCss) && outsideCss.includes('--shared-source-list-height') && outsideCss.includes('--shared-target-list-height') && outside.includes('sharedUploadTree(visibleItems') && outside.includes('sharedUploadTree(items'), 'shared directories lost the aligned flow tracks, responsive stacking, or tree hierarchy');
