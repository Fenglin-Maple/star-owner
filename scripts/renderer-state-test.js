...
(async () => {
  ...
  assert(outside.includes("if (event.localToolbox) { localRefreshGate.next()") && outside.includes('if (event.multiPart) {') && outside.includes('multipartRefreshGate.next();'), 'push events did not invalidate their matching in-flight toolbox state');
  ...
  const internalAgentManager = fs.readFileSync(path.join(__dirname, '..', 'src', 'core', 'internal-agent-manager.js'), 'utf8');
  const toolRunner = fs.readFileSync(path.join(__dirname, '..', 'src', 'core', 'tool-runner.js'), 'utf8');
  const storeSource = fs.readFileSync(path.join(__dirname, '..', 'src', 'core', 'store.js'), 'utf8');
  const app = fs.readFileSync(path.join(__dirname, '..', 'src', 'renderer', 'app.js'), 'utf8');
  assert(main.includes('multipartHotEvent') && main.includes("['stream', 'multipart-progress', 'session-updated']") && multipartManager.includes('MULTIPART_PROGRESS_EMIT_INTERVAL_MS = 800'), 'multi-part token events are still sent to the renderer without throttling');
  assert(main.includes('highFrequencyMultipartEvent') && main.includes("['stream', 'session-updated', 'multipart-progress']"), 'internal multi-part progress is still exported into the persistent activity log');
  assert(main.includes("if (event.type !== 'multipart-progress') {") && main.includes('publishEvent(activity);') && main.includes("'multipart-progress', 'shared-upload-progress'") && main.includes('const { cacheState, localToolbox, multiPart, ...activity }'), 'relayed multi-part progress can still enter global activity persistence');
  assert(app.includes("'multipart-progress'") && outside.includes("if (event.type === 'multipart-progress' && event.parent)") && outside.includes('updateMultipartProgressView(event.parentId || event.parent.id);'), 'multi-part progress still invalidates the global snapshot or rebuilds the complete tool UI');
  assert(outside.includes('multipartCollectionSignature') && outside.includes('data-multipart-part-progress-bar') && outside.includes('data-multipart-parent-progress'), 'multi-part viewer is missing stable selectors or collection option caching for incremental progress');
  assert(multipartManager.includes('parentState(parentId)') && multipartManager.includes("this.emit({ type: 'multipart-progress', parentId: key, parent })") && !multipartManager.includes("type: 'multipart-progress', parentId: key, multiPart:"), 'live multi-part progress still serializes the complete parent tree');
  assert(multipartManager.includes('partTasksByParent') && multipartManager.includes('activeSessionsByParent'), 'lightweight multi-part state does not group task or session scans');
  assert(storeSource.includes('batchSave(callback)') && multipartManager.includes('this.store.batchSave(() =>') && multipartManager.includes('Promise.allSettled(startPromises)'), 'multi-part worker startup does not coalesce synchronous full-database persistence');
  assert(internalAgentManager.includes('multipartSessionFlushDelayMs = 4000') && internalAgentManager.includes("dirtySessions.every((session) => session.mode === 'multipart')") && toolRunner.includes('multipartVolatileRunFlushDelayMs = 4000') && toolRunner.includes("multiPartRole === 'part'"), 'multi-part volatile session/tool progress still exports the complete database at the ordinary one-second cadence');
  assert(multipartManager.includes('partArtifactCache') && multipartManager.includes('partSummaryCache') && multipartManager.includes('cachedPartArtifact(task)'), 'multi-part index refresh still rereads every completed child artifact on each completion');
  assert(outside.includes('captureMultipartScrollState()') && outside.includes('restoreMultipartScrollState(scrollState)') && outside.includes('list.scrollTop = scrollState.partLists.get'), 'multi-part structural updates do not preserve parent-list and child-list scroll positions');
  const multipartActionHandler = outside.slice(outside.indexOf('async function handleMultipartAction'), outside.indexOf('function automaticSharedCollectionName'));
  assert(multipartActionHandler.includes('mergeMultipartParent') && multipartActionHandler.includes('removeMultipartParent') && !multipartActionHandler.includes('await refresh()'), 'multi-part actions still trigger a full outside-tool/global snapshot refresh');
  assert(main.includes('multiPartManager?.state({ reconcile: false })') && main.includes('return multiPartManager.state({ reconcile: options.reconcile === true })') && main.includes('const { parent, collections, ...activity } = event;'), 'global snapshots/runtime updates still reconcile all multi-part artifacts or structural parent payloads still enter activity persistence');
  ...
  assert(index.includes('id="appTitleCn"') && index.includes('id="appTitleEn"') && index.includes('class="app-title-swash"') && (index.match(/class="shared-filter-chevron"/g) || []).length === 6 && (index.match(/class="shared-filter-control"/g) || []).length === 6, 'random wordmark structure or persistent shared filter controls are missing');
  ...
  const wordmarkVariants = app.slice(app.indexOf('const APP_WORDMARK_VARIANTS'), app.indexOf('function randomizeAppWordmark'));
  assert((wordmarkVariants.match(/\{ id:/g) || []).length === 6 && app.includes('Math.floor(Math.random() * APP_WORDMARK_VARIANTS.length)') && app.includes('randomizeAppWordmark();'), 'the six wordmark variants are not selected once during renderer startup');
  assert(styles.includes('.app-title b {') && ['"Segoe Script"', '"Brush Script MT"', '"Bodoni MT"'].every((font) => styles.includes(font)) && styles.includes('[data-wordmark-variant="owner-brush-swash"] .app-title-swash') && styles.includes('body.theme-day .app-title') && styles.includes('body.theme-mint .app-title') && styles.includes('body.theme-bili .app-title') && styles.includes('--app-title-en-color: #23ade5;') && styles.includes('body.theme-endfield .app-title') && styles.includes('--app-title-en-color: var(--signal);'), 'wordmark fonts, SVG swash, or title-bar-aware theme colors are missing');
  assert(styles.includes('.overview-layout > .activity-panel { flex: 0 1 auto; min-height: 176px; height: clamp(176px, 24vh, 220px);') && styles.includes('.overview-layout > *:not(.activity-panel) { flex: 0 0 auto; }') && styles.includes('.overview-layout:has(.quickstart-disclosure[open]) > .activity-panel'), 'responsive recent-status height is missing');
  ...
  const aiCss = fs.readFileSync(path.join(__dirname, '..', 'src', 'renderer', 'ai.css'), 'utf8');
  ...
  const singleStartFlow = ai.slice(ai.indexOf('async function startSingleTask()'), ai.indexOf('function requestDuplicateDecision'));
  const sessionActionFlow = ai.slice(ai.indexOf('async function handleSessionAction'), ai.indexOf('function sessionButton'));
  const agentCreateFlow = ai.slice(ai.indexOf('async function createAgent'), ai.indexOf('function populateProviderSelect'));
  const startActionLabelFlow = ai.slice(ai.indexOf('function startActionLabel'), ai.indexOf('function dependencyStatus'));
  assert(singleStartFlow.includes('notifyBilibiliCookieRequired(error, notify)') && sessionActionFlow.includes('notifyBilibiliCookieRequired(error, notify)') && agentCreateFlow.includes('notifyBilibiliCookieRequired(error, notify)'), 'single-video or queue-Agent create/retry failures do not show the bottom-right Bilibili login notice');
  assert(startActionLabelFlow.includes("if (session.status === 'waiting-login') return '登录后重试';"), 'queue Agent waiting for B站 login does not expose the retry action label');
  ...
  assert(ai.includes('<small class="dependency-release-hint">点击依赖标题可以跳转到单独下载依赖包的网址，手动进行本地导入</small>') && ai.indexOf('dependency-release-hint') < ai.indexOf('<p></p>', ai.indexOf('dependency-release-hint')) && aiCss.includes('grid-template-rows: auto auto minmax(0, 1fr) auto') && aiCss.includes('.dependency-release-hint'), 'dependency manual-download guidance is missing, misplaced, or not included in the stable row layout');
