...
(async () => {
  ...
  assert(styles.includes('.app-title b {') && styles.includes('font-style: italic;') && styles.includes('body.theme-bili .app-title b { color: #ffffff; }') && styles.includes('body.theme-endfield .app-title b { color: var(--signal); }') && styles.includes('.overview-layout > .activity-panel { flex: 0 1 auto; min-height: 176px; height: clamp(176px, 24vh, 220px);') && styles.includes('.overview-layout > *:not(.activity-panel) { flex: 0 0 auto; }') && styles.includes('.overview-layout:has(.quickstart-disclosure[open]) > .activity-panel'), 'brand title theme contrast or responsive recent-status height is missing');
