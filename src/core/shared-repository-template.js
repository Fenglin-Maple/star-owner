...
jobs:
  catalog:
    ...
    steps:
      ...
      - name: Commit catalog with protected-branch handoff
        env:
          GITHUB_TOKEN: \${{ github.token }}
        run: node scripts/publish-shared-catalog.mjs
`),
    textFile('scripts/validate-shared-docs.mjs', VALIDATOR_SCRIPT),
    textFile('scripts/build-catalog.mjs', CATALOG_SCRIPT),
    textFile('scripts/publish-shared-catalog.mjs', PUBLISH_CATALOG_SCRIPT),
...
const PUBLISH_CATALOG_SCRIPT = `import { execFileSync } from 'node:child_process';
