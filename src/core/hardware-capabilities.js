...
const { DEFAULT_ASR_MODEL, getAsrModel, normalizeAsrModel } = require('./asr-models');
const { PROJECT_ROOT } = require('./workspace');

const execFileAsync = promisify(execFile);

async function detectAsrHardware({ gpu = {}, model = DEFAULT_ASR_MODEL, projectRoot = PROJECT_ROOT } = {}) {
...
function evaluateAsrHardware(input = {}) {
  ...
  const selectedModel = normalizeAsrModel(input.model);
  ...
  return {
    ...
    model: {
      ...
      gpuComputeType: modelDefinition.gpuComputeType,
      cpuComputeType: modelDefinition.cpuComputeType,
      minimumGpuMiB: modelDefinition.gpuTotalMiB,
      startupFreeMiB: modelDefinition.gpuStartupFreeMiB,
      minimumCpuMemoryMiB: modelDefinition.cpuMemoryMiB
    },
