...
function renderTaskRows(tasks) {
  ...
  for (const task of tasks) {
    ...
    const metadataIncomplete = Boolean(task.metadataIncomplete);
...
function renderRunHistoryItem(run) {
  ...
  const progressSummary = run.asrProgress
    ? `ASR ${Math.round(Number(run.asrProgress.progress || 0) * 100)}%${run.asrProgress.attempt ? ` · 尝试 ${run.asrProgress.attempt}/${run.asrProgress.totalAttempts || 3}` : ''} · ${run.asrProgress.phase || '-'} · ${Number(run.asrProgress.audioSeconds || 0).toFixed(1)}s / ${Number(run.asrProgress.totalSeconds || 0).toFixed(1)}s`
    : queueSummary;
