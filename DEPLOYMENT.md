...
Existing v1.0.3-or-newer portable installations can instead download `Star-Owner-Updater-v<version>-win-x64.exe`, close the old application and all Agents, run the updater, and select the old project root. The self-contained updater copies itself to a managed temporary directory, reads only the stable GitHub `latest` Release, resumes the core download, verifies SHA-256, safely extracts the archive, and then hands off to the same journaled replacement transaction used by the application. It keeps the absolute project path and preserves `workspace/`, `.cache/`, `runtime/models/`, `runtime/python/`, `runtime/faster-whisper/` and `runtime/vc-runtime/`; `runtime/git/` is refreshed with the application core. A cancellation during download/verification/extraction leaves the old project untouched. A cancellation after handoff restores every changed path from the transaction backup.
...
8. For a code-only Release, upload the core ZIP, standalone updater EXE, and both checksums. Keep `dependencyReleaseVersion` pinned to the compatible dependency Release; publish new model/runtime assets only when their content or layout changes.
