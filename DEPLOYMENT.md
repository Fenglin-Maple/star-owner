...
Existing v1.0.3-or-newer portable installations can instead download `Star-Owner-Updater-v<version>-win-x64.exe`, close the old application and all Agents, run the updater, and select the old project root. Version `1.7.3` introduces a strict same-version contract: application vX.Y.Z may be installed only by updater vX.Y.Z. The self-contained updater copies itself to a managed temporary directory and reads only the stable GitHub `latest` Release. If it is older than latest, it downloads and verifies the target version's updater first, hands the selected project path to that executable, and exits without downloading or installing the future core package itself. The target updater then resumes and verifies the matching core archive, safely extracts it, and hands off to the journaled replacement transaction. It keeps the absolute project path and preserves `workspace/`, `.cache/`, `runtime/models/`, `runtime/python/`, `runtime/faster-whisper/` and `runtime/vc-runtime/`; `runtime/git/` is refreshed with the application core. A cancellation during updater/core download, verification or extraction leaves the old project untouched. A cancellation after transaction handoff restores every changed path from the backup.

Already-published v1.7.2 and earlier binaries cannot contain the new bootstrap protocol. For the first upgrade from one of those versions to v1.7.3 or newer, download the updater whose version exactly matches the stable target Release. Once an installation has entered v1.7.3 or newer, an older updater can bootstrap to the current target updater automatically.
...
`1.7.1` is a code/documentation-only portable updater reliability update and keeps the published `v1.0.0` runtime/model assets unchanged. A bundled native WinForms updater owns the visible update/migration lifecycle after a two-way `ready -> acknowledged -> accepted` handshake; Electron exits only after the updater has started its transaction helper. The window continuously animates the application icon, displays phase/item/progress and elapsed time, persists a UTF-8 diagnostic log, and exposes `中止并回退`. Cancellation checkpoints perform ordinary rollback; an unresponsive or abruptly terminated helper is stopped and recovered from the durable journal. Handshake launch/timeout failures leave the main application running. No SQLite schema migration or new model, Python, Git or Node asset is required; the compiled updater uses the .NET Framework component included with supported Windows 10/11 systems.

`1.7.2` extends that native executable into a separately downloadable updater for any complete v1.0.3-or-newer portable directory, including historical pre-releases. The EXE embeds the current apply/recovery helpers and icon, relocates itself outside the selected target to avoid file locks, rejects a running old application or damaged SQLite database, reads only stable `latest`, supports `.partial`/HTTP Range continuation, requires SHA-256, and extracts with traversal, ADS, device-name, link, case-collision, size and path-length checks. A second temporary updater process completes the two-way handoff before the preparation window exits. Release builds add the updater EXE and checksum beside the core ZIP; runtime/model assets and the v1.0.0 dependency baseline are unchanged.

`1.7.3` adds the strict same-version updater protocol without changing the v1.0.0 dependency baseline. Every stable application Release must publish the target core ZIP, target updater EXE and both SHA-256 files. Application-managed updates download and validate the updater before the core archive. Standalone updaters older than latest self-bootstrap to the matching target updater and transfer the selected installation path; they never install a future core directly. Transaction requests, the executing EXE identity, `UpdaterBuildInfo.cs`, package version and portable manifest are checked against one version and protocol before replacement. Core extraction determines and validates the common ZIP prefix before calculating output paths, strips that prefix into the shortest managed `.updates/s` staging directory, and rechecks final Windows file and directory limits. This preserves the project-root transaction boundary while providing more path budget for old installations in deep or non-ASCII directories. Durable journal/result publication uses unique temporary files, atomic replacement and bounded retries when a progress reader briefly holds the previous JSON; persistent failure still enters the ordinary rollback path. The updater source, version identity and build scripts remain tracked under `tools/updater/`.

### Updating and migrating an existing installation

For a portable installation, use `设置 -> 应用更新与迁移 -> 检查更新`. Only the GitHub `latest` stable Release is eligible. The application first downloads the target updater with retry and Range continuation, verifies its SHA-256 and embedded version/protocol, and only then downloads the matching core archive. It verifies the core SHA-256; rejects Win32 drive/UNC/device paths, traversal, NTFS alternate streams, control characters, dangerous segments, reserved names and case-insensitive collisions; strips only a validated common ZIP prefix into a short staging directory; and validates the staged transaction scripts, portable manifest and bundled updater identity before launch. The verified target updater displays the operation while a temporary helper from the new package replaces the complete application file set, including `templates` and project-local `runtime/git`. `workspace/`, the ASR/runtime directories other than `runtime/git`, and `.updates/` are retained. The helper records each successfully backed-up or originally absent path before replacement and never deletes an original that lacks a completed backup. Cancellation is checked before and after every managed top-level item; a helper that cannot reach a checkpoint is terminated before the independent recovery script restores the journaled backup. `Start-StarOwner.cmd` also runs `recover-portable-operation.ps1` before Electron, so an interrupted operation is rolled back first, while an unsafe or incomplete recovery blocks startup and preserves `.updates/operation-result.json` for diagnosis. Routine cleanup touches only recognizable updater/core downloads and partials, update staging directories and operation backups. It retains at most two recent verified files per download type, one resumable partial per type and two recent backups within their retention windows; active journals, prepared packages and failed-recovery evidence are protected. Installations at `1.7.2` or older should use the separately downloadable updater that exactly matches the stable target version for their first upgrade into this protocol. Manual file mixing is no longer required.
