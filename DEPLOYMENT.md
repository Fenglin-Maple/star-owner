...
9. Verified archives extract into staging and commit below `runtime/` with a durable per-entry journal. An interruption before commit restores every old runtime path; an interruption after commit keeps the complete new runtime and only finishes backup cleanup, preventing mixed dependency versions.
