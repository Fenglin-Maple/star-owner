...
Portable core updates and workspace migrations use a bundled native WinForms process copied outside the replacement tree before launch. The updater renders the branded animated status window and remains independent after Electron exits. Handoff is two-way: the updater writes `ready`, the main process atomically writes `acknowledged`, the updater starts the non-detached PowerShell transaction helper and writes `accepted`, and only then may the main process call `app.quit()`. A missing executable, early exit, helper-start failure or timeout therefore leaves the main application alive. The helper atomically publishes phase, current item, completed/total items and normalized progress with the durable backup metadata. Cancellation writes an operation-scoped request checked on both sides of every top-level copy; if a copy does not return promptly, the updater terminates the helper and invokes a separately copied recovery script. Success and verified rollback relaunch `Start-StarOwner.cmd`; rollback failure preserves the backup, journal/result and UTF-8 updater log for diagnosis. The same controller and transaction protocol serve updates and old-workspace migration.

The same executable also has a no-argument standalone mode for older portable installations. It first self-relocates to a unique directory below the current user's temporary StarOwner root, then lets the user select a complete v1.0.3-or-newer project root. The preparation service validates the product/version, write access, SQLite header, path budget and absence of project Electron/Node processes; it recovers a prior durable journal before continuing. Release resolution is pinned to the public stable `latest` endpoint. The core archive supports `.partial` and Range continuation, requires an asset digest or matching checksum asset, and is extracted with managed `ZipArchive` only after link, traversal, absolute/device/ADS path, reserved-name, case-collision, expanded-size and output-path validation. Preparation never modifies core files. After staged runtime validation, a second relocated native process completes the existing two-way handoff and runs the embedded current helper rather than trusting scripts from the selected old directory. Keeping the project root unchanged preserves Workspace records and path-derived Bilibili WebView identity; the transaction retains Workspace, caches, models, Python, faster-whisper and VC++ runtime while refreshing core files and Portable Git.
