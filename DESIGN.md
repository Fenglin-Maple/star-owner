...
Ordinary users receive GitHub Release portable archives assembled by `scripts/build-portable-release.ps1`. The recommended `small` bundle contains every runtime tool needed for normal operation under project-relative paths; no global Node, Python, FFmpeg, yt-dlp, CUDA Toolkit, or model download is required. The optional quality model is published separately when necessary to stay under GitHub's per-asset size limit.
