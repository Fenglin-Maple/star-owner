...
Each claimed video starts a fresh provider request context containing only the fixed system contract and that video's budgeted materials. Previous-video messages are never appended. The persistent Worker ID, aggregate usage, counters, and logs survive across videos, while every task has a new `workId` and increments the session context-cycle counter. Before inference, the manager reserves protocol, vision-image, and output capacity against the configured model context window. If subtitles, ASR, metadata, comments, template, or a validation-repair draft exceed the remaining input budget, section-aware limits reduce the largest sources and record a visible compaction. A provider context-limit error triggers one aggressive fresh-request retry with fewer images and tighter source budgets while preserving Worker/work identity. A second failure uses the ordinary abort/cleanup path instead of carrying a partial checkpoint into the next task.
