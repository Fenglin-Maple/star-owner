...
Portable core updates and workspace migrations use a native WinForms process copied outside the replacement tree before launch. Updates enforce one identity across the target application, executing updater, transaction request and protocol declaration: vX.Y.Z may be installed only by updater vX.Y.Z. The updater identity is compiled from the tracked `tools/updater/UpdaterBuildInfo.cs`; build and package verification compare it with `package.json`, executable product metadata and `portable-manifest.json`. The updater renders the branded animated status window and remains independent after Electron exits. Handoff is two-way: the updater writes `ready`, the main process atomically writes `acknowledged`, the updater starts the non-detached PowerShell transaction helper and writes `accepted`, and only then may the main process call `app.quit()`. A missing executable, version/protocol mismatch, early exit, helper-start failure or timeout therefore leaves the main application alive. The helper publishes phase, current item, completed/total items and normalized progress with durable backup metadata through unique temporary files and atomic replacement. A bounded retry absorbs short read locks from progress observers; failure after that bound still invokes ordinary transaction rollback. Cancellation writes an operation-scoped request checked on both sides of every top-level copy; if a copy does not return promptly, the updater terminates the helper and invokes a separately copied recovery script. Success and verified rollback relaunch `Start-StarOwner.cmd`; rollback failure preserves the backup, journal/result and UTF-8 updater log for diagnosis. The same controller and transaction protocol serve updates and old-workspace migration.

The same executable also has a no-argument standalone mode for older portable installations. It first self-relocates to a unique directory below the current user's temporary StarOwner root, then lets the user select a complete v1.0.3-or-newer project root. The preparation service validates the product/version, write access, SQLite header, path budget and absence of project Electron/Node processes; it recovers a prior durable journal before continuing. Release resolution is pinned to the public stable `latest` endpoint and requires the target core ZIP, target updater EXE and both SHA-256 values. If the running updater is older than latest, it downloads and probes the target updater, transfers the selected project path through an authenticated temporary handoff, and exits; only the target updater may fetch or install its matching core. The core archive supports `.partial` and Range continuation, requires an asset digest or matching checksum asset, and is extracted with managed `ZipArchive` only after link, traversal, absolute/device/ADS path, reserved-name, case-collision and expanded-size validation. The unique common top-level ZIP prefix is determined before any output path is constructed, then stripped into the shortest managed project-local `.updates/s` path. Real file and parent-directory paths are checked against their separate Windows limits before extraction. Keeping staging under the project root preserves the transaction helper's trust boundary while avoiding avoidable version/prefix path components. Preparation never modifies core files. After staged runtime and version-contract validation, a second relocated native process completes the existing two-way handoff and runs the embedded target helper rather than trusting scripts from the selected old directory. Keeping the project root unchanged preserves Workspace records and path-derived Bilibili WebView identity; the transaction retains Workspace, caches, models, Python, faster-whisper and VC++ runtime while refreshing core files and Portable Git. Already-published v1.7.2 and earlier updaters require a one-time manual download of the target updater because those binaries predate bootstrap support; v1.7.3 and newer updaters can self-bootstrap forward.

Release discovery and version-directed installation are separate updater contracts. Discovery initiated by an installed application or any older updater queries only the stable GitHub `latest` endpoint and must ignore drafts and pre-releases. A bootstrap-capable old updater may hand off to the exact updater attached to a newer stable latest, but may not install that future core itself. In contrast, an updater explicitly downloaded from a specific stable or pre-release page enters pinned-target mode: it resolves only its own embedded version and exact Release/tag assets, requires the selected v1.0.3-or-newer project to be older, and installs only that matching version. This allows a user to opt into a pre-release without making it visible to automatic update checks. Every stable and pre-release page must therefore ship a matching core ZIP, updater EXE and both checksum files. Pinned-target pre-release resolution remains a required post-v1.7.3 extension; v1.7.3 implements stable-latest discovery and same-version bootstrap but not direct pre-release targeting.
...
High-frequency multi-P session state, tool progress and child-process logs are updated in memory and flushed in bounded batches; multi-P volatile session/tool progress uses a four-second window, while ordinary Agent persistence, stage transitions, cancellation, terminal results and shutdown recovery retain their existing immediate or short-window behavior. Starting several multi-P workers coalesces the synchronous session, claim and tool-run SQLite exports into one save. A failed child P is represented as a disabled pending task with an explicit retry reason, and the parent viewer exposes individual and batch continue/retry controls. The parent index prefers the P1 cover/first keyframe and falls back safely when that asset is unavailable. Shared-tool mount actions sit beside remote catalog loading, while the remote tree, local mount tree and upload candidates use the same nested visual hierarchy.

## 18. Supported Video Boundary

The ordinary single-video and batch Agent workflows accept ordinary Bilibili BV videos with exactly one page. For queue Agent tasks whose collection is a real Bilibili favorite collection (`isBiliCollection`), the metadata stage additionally requires a non-empty `pages` list and, when `videos` is present, a matching count. Missing, empty or incomplete metadata receives at most three total `/x/web-interface/view` requests; after exhaustion the current attempt is cleaned, the task is disabled and returned to pending with a retryable metadata-exception marker, and the queue counts it as skipped before continuing. This completeness guard is Agent-only and source-scoped: internal single-video collections, the dedicated multi-part collection, local video/audio imports and downloaded-video cache collections do not receive `--metadata-retries` and retain their existing paths. Once metadata is complete, the existing support inspection still rejects more than one `pages` entry as `UNSUPPORTED_VIDEO_TYPE`, so the new guard does not replace the normal multi-part decision. Inputs under Bilibili bangumi (`ep/ss/md`), cheese, festival, audio and live routes are rejected before URL-to-BV resolution. Favorite entries without a BV are retained as disabled inventory records with a visible reason. The independent “B站多P视频总结” tool now handles one standard multi-part BV parent at a time: it persists CID-based P children, limits internal concurrency, writes a deterministic parent index, supports stop/continue, refresh/append and parent deletion, and keeps completed P outputs after interruption. The parent viewer can flatten each parent into per-P rows with live progress, phase and CID, collapse back to the aggregated parent progress, and stop one active or pending P without affecting siblings; a failed replacement worker is reported as a warning while the stopped P remains safely rolled back. Provider rate-limit, concurrency and temporary capacity errors use bounded backoff retries, while explicit credential/parameter errors stop immediately. During streaming, high-frequency P events only schedule a throttled viewer update; they do not scan the full task/session store, persist a task for every token or send multipart model text to unrelated Agent pages. The parent index copies only each completed child's normalized `## 小结` section, rebases its relative resources, and links the complete child Markdown; startup deterministically upgrades existing local indexes without another model request. Multi-part and special pages remain rejected by the single-video and batch Agent entry points.

The `pages` completeness validator is an opt-in, reusable `video-tool` capability: `--metadata-retries` enables it, while callers that omit the option retain their previous behavior. The current Agent-only activation is a product scope decision rather than a technical limitation. User reports may later justify enabling the same guard for ordinary single-video summaries or the remote-metadata stage of Bilibili video downloads. Any expansion must first add entry-specific regression coverage for Cookie refresh, risk-control handling, multi-part routing, local/cache bypass and attempt rollback. The dedicated multi-part workflow must continue using its own parent metadata contract instead of inheriting the ordinary single-page guard.
...
Multi-part live progress is a dedicated volatile channel. It is emitted at most once per 800 milliseconds, carries only the changed parent, uses one task scan and one Agent-session scan, skips filesystem artifact reconciliation, and never enters persistent activity history or the global snapshot invalidation path. Ordinary global snapshots and runtime updates request lightweight multi-part state; explicit repair reads and startup reconciliation still verify real artifacts, grouping all parents into one task/session scan. The Renderer retains collection option nodes, keeps parent ordering stable during patches, updates only the affected parent's progress controls for live events, and preserves both the parent-list and nested child-list scroll offsets across structural renders. Stop, continue, retry, refresh and delete merge their returned parent locally instead of refreshing the other “B站之外” tools and the global snapshot. Parent-index generation caches unchanged artifact resolution and extracted child summaries, so each completed P does not reread every older Markdown. Existing databases run a one-time migration that removes legacy `multipart-progress` activities, strips redundant multi-part trees from terminal activity records and vacuums reclaimed pages. Multi-part parent artifact deletion uses the same containment/link policy as other destructive operations but yields through asynchronous filesystem calls so the Electron main loop stays responsive. Its child Agent-session and Worker mutations are accumulated without per-session persistence and committed in one final SQLite save; ordinary Agent and single-video session deletion keeps its immediate durable default. After the deleted row is structurally rerendered, focus returns to the surviving multi-part collection selector instead of a detached action button.
...
`npm run verify:release` checks package/lock versions, updater source/package/executable identity, machine-specific paths, JavaScript/Python/C# updater inputs, all integration tests including local media/document tools, multi-part/shared knowledge, Git/Node/Python runtime isolation, both ASR models and npm audit. The update/migration suite launches the real native updater across a terminating parent process and Chinese/space paths, verifies target-updater-first download order, embedded version probing, short stripped staging paths, two-way handoff, successful replacement, mid-apply cancellation rollback, abrupt-helper recovery, startup-journal recovery, migration and retained runtime/workspace boundaries. The standalone-updater suite additionally compiles a simulated older updater and uses a local ranged HTTP Release fixture to verify automatic same-version bootstrap, selected-path transfer, old/prerelease inspection, running-process and damaged-SQLite refusal, stable-only resolution, unsafe archive rejection, interrupted-journal recovery, partial continuation, preparation cancellation, end-to-end handoff/update and preserved Workspace/runtime/model/private-Git boundaries. `test:library-auth-ui` additionally starts an isolated Electron fixture and verifies Bilibili login guidance, local-media download-target exclusion, and task-state-independent video/document library display. `test:runtime-node` deliberately removes the global `PATH` and verifies that the bundled Electron Node mode still executes the video tool; `test:runtime-isolation` checks controlled PATH/environment handling and project-owned child-process boundaries; `test:git-runtime` verifies that global Git config and external Git paths are rejected. Shared-knowledge tests additionally cover private authorized reads, owner/Fork routing, repository-contract validation and registry persistence, repository-specific mounts, repository-isolated batch synchronization, visible operation progress, actual default-branch templates, upload limits/cancellation, repository file allowlists and cross-repository multi-part isolation.

Every application Release candidate, stable or pre-release, also has a mandatory destructive-test matrix performed only on disposable copies in an external local directory. The matrix always includes v1.0.3, v1.6.2 and one historical pre-release between them; one post-v1.6.2 pre-release is optional additional coverage. Each required source is updated through the real native transaction to the exact candidate assets, with before/after fingerprints for Workspace, SQLite, generated documents, login state, private GitHub credentials, models, caches and retained runtime. The gate includes cancellation before replacement and cancellation during replacement with verified rollback. A pre-release candidate additionally proves direct pinned installation to itself and proves that ordinary discovery still ignores it in favor of stable `latest`. Candidate reports and project copies remain outside Git and do not alter historical Release pages.
