...
Project-local runtime and models may be installed from GitHub Release assets. Installation uses staging, backup, SHA-256 verification, a transaction journal and startup rollback. Archives may write only below `runtime/`. Application and dependency versions are independent: `package.json.dependencyReleaseVersion` is the compatibility contract used by the manager, portable manifest and API-rate-limit direct fallback. Version `1.0.1` pins the unchanged runtime, small and medium assets from dependency baseline `1.0.0`.
