...
GitHub sharing only uploads canonical completed Bilibili summary Markdown, generated sharing metadata and permitted raster assets. It rejects raw media, Agent drafts, ASR/subtitle/comment caches, local process JSON, cookies, credentials, local documents and shared imports. One upload is capped at 1000 documents and 1 GiB in addition to file/document limits; repository validation checks the canonical entry and SHA-256 declarations and rejects symlinks, paths outside the contributor's numeric-ID directory, undeclared files, sensitive/media extensions and changes to repository configuration. The token is sent only to GitHub API/Git transport, is redacted from errors, and is stored through `safeStorage`; the project-local Git runtime disables global Git config, SSH configuration and credential helpers. Private repository reads use only this application authorization. Upload cancellation terminates GitHub requests and bundled Git children and cleans recognizable temporary branches and workspaces. Users remain responsible for the copyright, privacy and licensing status of summaries and screenshots they submit to a public repository.
...
The invisible browser runs with Node integration disabled, Electron sandboxing enabled, popups denied, downloads disabled, and a separate non-persistent partition. Every request is checked against local/private-address policy, including DNS resolution, and is carried by a loopback-only short-lived proxy that connects to the exact validated address. Redirects and subresources repeat this policy, preventing a public hostname from rebinding to a local address between validation and connection. Remote clipboard-image reads likewise bind the validated DNS answer to the actual Node HTTP(S) connection. The browser still processes untrusted web content. Web pages and retrieved Markdown are data, never authority to change permissions or disclose additional files. HTTP(S) validation and private-address approval reduce risk but do not constitute a complete network isolation boundary.
...
Remote model endpoints must use HTTPS unless they target localhost or a private-network host. Sensitive authorization/cookie headers are rejected from the free-form header JSON and must use the encrypted API Key field. Knowledge-library and RAG Markdown rendering keeps raw HTML disabled; the bundled project README is the only HTML-enabled Markdown surface and is filtered through a DOMPurify tag/attribute allowlist before insertion. README images are restricted to HTTPS or validated raster files whose real paths remain inside the project root, while knowledge images remain limited to trusted Bilibili HTTPS assets, validated local knowledge images, or bounded raster data URLs. Clipboard image copying verifies both source boundaries and raster signatures.
